 ...
 *
 * $Id: ftntoss.c,v 1.1.1.1 2003/09/23 17:45:11 rusfidogate Exp $
 *
 ...
 */
void do_seenby(LON *seenby, LON *nodes, LON *new, LON *passive, int cup, Node *from)
...
int do_echomail(Packet *pkt, Message *msg, MsgBody *body)
{
	...
	if ( autocreate_ng )
	{
	    ...
	    if ( NULL != (autocreate_area = areas_lookup( areaname, NULL, &pkt->to)))
	    {
...
area have status read-only\r\n\r\n\
    Your Message:\r\n\r\n\
		...
		Subject: %s\r\n\r\n",
			znfp1(&msg->node_from),
			area->area,
			nf1(&msg->node_from), znfp1(&pkt->to),
			msg->subject);
...
area have status 'W' (no traffic from uplink)\r\n\r\n\
	...
	if(areasbbs_isstate(area->state, 'P'))
    	    return do_bad_msg(msg, body);
    }
    area->time = time(NULL);
    areasbbs_changed();
    ...
    (area->msgs_out)++;
    (area->msgs_size)+=sizeof(body)+sizeof(msg)+sizeof(seenby)+sizeof(path);
