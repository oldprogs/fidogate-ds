 ...
 *
 * $Id: ftnpack.c,v 1.1.1.1 2003/09/23 17:44:44 rusfidogate Exp $
 *
...
Packing *packing_parse_line(char *buf)
{
	...
	if(!p)
	{
	    log("lastarcmailchars: argument missing");
	    return NULL;
	}
...
int arcmail_search(char *name, long max_arc, long psize)
{
    ...
    *p = C[last_arcmail_chars];
    if(check_access(name, CHECK_FILE) == TRUE && (size = check_size(name)) == 0)
    {		/* Switch to the next cicle C[last_arcmail_chars] -> C[0] */
...
int main(int argc, char **argv)
{
	...
	{
	    log("ERROR: %s: invalid BundleDisp value", CONFIG);
	    exit_free();
	    exit(EXIT_ERROR);
	}
    } else
