 ...
 *
 * $Id: routing.c,v 1.2 2004/03/01 19:00:54 rusfidogate Exp $
 *
...
PktDesc *parse_pkt_name(char *name, Node *from, Node *to)
{
    ...
    /* Sanity check */
    if(desc.type!=TYPE_ECHOMAIL && desc.type!=TYPE_NETMAIL && desc.type!=TYPE_TICK)
    {
	desc.grade = '-';
	desc.type  = '-';
