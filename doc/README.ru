  ...
  $Id: README.ru,v 1.26 2004/11/22 23:09:39 anray Exp $
...
`AI_1'(--enable-ai1)
    ...
    HOSTS для данного отправителя. Генерация ^MSGID также использует
...
`SEP_RFC2FTN_PKT'(--enable-sep-pkt)
...
# Каталог, в котором хранятся динамические библиотеки, нужные для работы fidogate
#(если они есть т.е. все бинарники скомпилены с подключением динамических библиотек).
...
# ftnaf программой, если ареафикс работает через MTA, а не фейкового поинта).
AreaFixMailer	/usr/sbin/sendmail -t

# Адрес, который добавляется в CC ответов ареафикса (используется совмесно с
# AreaFixMailer).
...
# Список адресов, которые подписываются автоматически на создаваемые файлэхи.
...
# Если в заголовке pkt содержится поинтовый адрес, не совпадающий с адресом
# в ориджине, или если этого адреса нет в кладже ^MSGID, то сообщение считать
...
# Ключи, которые передаются файлфиксу при автокриэйте файлэхи в fareas.bbs. 
