  ...
  $Id: README.ru,v 1.1 2003/09/26 16:51:30 rusfidogate Exp $
...
2.  Файлы конфигурации и их описание.
    ...
    Вовсех конфигах fidogate действует директива "include", указывающаяя что
...
необходимо наличие установленной программы cvs и ssh, выполнить команду:
...
`AI_1'
    ...
    если Message-Id отсутствует. Используется только вместе с одной из директив
...
`REPLY_IF_X_FLAGS_M'
    При вставлении в заголовок `X-flags:' параметра `m', при гейтовании RFC->FTN
    добавлять кладж `^AREPLY'.
...
`AF_AVAIL'
    ...
    команде %avail список будет выводится список конференций имеющих отличный от
    вышеприведенных статус, либо всех, если определен параметр 
    `AreaFixAvailPrintsAllAreas' в fidogate.conf.
...
`DEFAULT_OUTNETMAIL'
    Путь к каталогу, в который будет складываться нетмейл.
...
`OLD_BATCHER'
    Старый тип ведения батч фала ньюс сервера (вчастности его использует
    INN 2.3).
...
эхоконференциям и определяется наш aka.

 Опции правила:
     -a Z:N/F.P         Альтернативный адрес
     -z ZONE            Альтернативный адрес зоны
     -d DISTRIBUTION    Указание Distribution заголовка для данной ньюсгруппы
     -o ORIGIN          * Origin
     -g                 Запрещение гейтования писем FTN->RFC
     -l                 Разрешение только локальног кросспостинга
     -x                 Запрещение кросспостинга
     -8                 Сообщения в 8bit ISO-8859-1 кодировке
     -H                 Приводить к соответствию имена news и эхоконференции
     -!                 Запрещение гейтования конференции FTN<->RFC
     -R LVL             Задание варианта ^ARFC заголовков
     -m MAXSIZE         Максимальный размер сообщения для данной эхоконференции
                        RFC->FTN (если 0, то не проверяется)
     -X "Xtra: xyz"     Добавлять указанный RFC заголовок FTN->RFC
                        (множественное указание опции -X разрешено)
...
""		fido.			    -8 -H -z 2 -a 2:5030/468.999
    ...
    Первое поле - название эхоконференции, второе - название название news
группы, третье - опции. В данном примере в правиле 4 присутствует указание,
...
правило игнорируется.
    Указание ключа -z Z требуется ДЛЯ КАЖДОГО правила, рекомендуется
исполязовать явное указания AKA адреса посредствам ключа -а Z:N/F.P. При
...
# поле "From:" при ответе на запрос.
#FileFixName	MyFFix
...
# статус `U' (не выписана у аплинка). Таймаут задается в днях.
#
...
# если стоял статус `F'. Таймаут задается в
# днях
#
...
# Время, втечении которого содержится, база соответствий нетмейловых MSGID и
# RFC-MSGID, в днях.
# (decimal fraction allowed)
MaxHistoryDBC 28
...
# Ключи, которые передаются ареафиксу при создании файлэхи в fareas.bbs. 
#AutoCreateFileechoLine -# -k A

# Путь для создания файлэховых каталогов
...
# Отключение вставки в FTN сообщения кладжей `^ARFC'.
NoRfcKludge
...
2.10	SPYES
    ...
    Конфиг для указания скртого форварда нетмейла, без добавления скрытого
...
2.11	UPLINKS
    ...
    Конфиг, для описания аплинков при форвардреквестах.
...
options - опции через пробел:
    ...
    -l Level	- уровень доступа к конференции. Если линк имеет уровень доступа
		  в passwd файле числинно меньший чем указанный в данном поле,
...
==============================================
	...
	delete		удалить "мертвые" конференции (не имеющие аплинков или
			даунлинков).		(устаревшая функция)
